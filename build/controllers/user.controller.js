...
exports.updateAccessToken = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const decoded = jsonwebtoken_1.default.verify(refresh_token, process.env.REFRESH_TOKEN_SCRET);
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 604800); //7days in seconds
        return next();
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 400));
